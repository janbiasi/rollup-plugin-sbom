...
describe("Rolldown V1", () => {
    ...
    test("it should autodetect the root application correctly", async () => {
        ...
        expect(metadata.component.name).toEqual("rolldown-v1");
        expect(metadata.component.group).toEqual("@fixtures");
    });

    test("it should detect all used components correctly", async () => {
    ...
    test("it should represent depending relation on parent modules correctly", async () => {
    ...
    test("should detect rolldown as tool correctly", async () => {
        const { metadata } = await helpers.getCompiledFileJSONContent("plugin-outdir/filename.json");
        const rolldownTool = metadata.tools.find((entry) => entry.name === "rolldown");

        expect(rolldownTool).toBeDefined();
    });
});
